...
function PlaceableHusbandry:setOwnerFarmId(superFunc, farmId, noEventSend)
    ...
    if spec.loadingStation ~= nil then
        spec.loadingStation:setOwnerFarmId(newFarmId, true)
    end
    if spec.unloadingStation ~= nil then
        spec.unloadingStation:setOwnerFarmId(newFarmId, true)
    end
end
...
-- @param float deltaFillLevel
-- @param integer fillTypeIndex
-- @return float remaining delta
