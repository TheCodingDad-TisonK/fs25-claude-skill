...
function IngameMapElement:moveCenter(x, y)
    ...
    if Platform.isMobile then
        ...
        self.mapCenterX = math.clamp(self.mapCenterX + x, width * -0.25 + self.originalMapCenterX, width * 0.25 + self.originalMapCenterX)
        ...
        if self.limitCursorMovement then
            ...
            self.mapCenterY = math.clamp(self.mapCenterY + y, height * -0.25 + self.originalMapCenterY, height * 0.25 + self.originalMapCenterY)
        end
    end
...
---Pan to a hotpspot, if possible the hotspot will end up at the center of the screen, or if it is near the map border it will be centered as much as possible
-- @param hotspot the hotspot that gets centered
...
---
-- @param zoomTarget if set, instead of zooming with predefined speed, map will instantly zoom in to that zoom level
...
function IngameMapElement:updateCursor(deltaX, deltaY, dt)
    if self.cursorElement ~= nil then
        ...
        if Platform.isMobile then
            ...
            self.cursorOffsetX = math.max(math.min(self.cursorOffsetX, mapLayout.width * 0.5 - cursorSize[1] * 0.5), -mapLayout.width * 0.5 + cursorSize[1] * 0.5)
...
-- @param float offset offset between new distance and distance of last pinch event
-- @param pinchCenterX x position of the center point of the pinch
-- @param pinchCenterY y position of the center point of the pinch
-- @param float distance distance between both touch positions
