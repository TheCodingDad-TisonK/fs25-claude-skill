...
---
function MultiTextOptionElement:clone(parent, includeId, suppressOnCreate)
    local ret = MultiTextOptionElement:superClass().clone(self, parent, includeId, suppressOnCreate)
...
function MultiTextOptionElement:touchEvent(posX, posY, isDown, isUp, touchId, eventUsed)
    if self:getIsActive() then
        if MultiTextOptionElement:superClass().mouseEvent(self, posX, posY, isDown, isUp, touchId, eventUsed or self.wasContinuousTrigger and isUp) or self.wasContinuousTrigger and isUp then
            eventUsed = true
        end
